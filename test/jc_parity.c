...
static void usage(const char *prog) {
	fprintf(stderr,
			"Usage: %s [-n cases] [-s seed] [-a avg] [-m min] [-x max] [-l max_input_len] [-j jump_mto] [-b mask_bits]\n",
			prog);
}
...
static size_t chunk_stream_cpu(const unsigned char *buf, int len, int *out, int out_cap) {
	...
	while (offset < len && (int)num < out_cap) {
		int csz = gearjump_chunk_data((unsigned char *)buf + offset, len - offset);
...
static size_t chunk_stream_gpu(const unsigned char *buf, int len, int *out, int out_cap) {
	...
	size_t num = 0;
	while (offset < len && (int)num < out_cap) {
		int csz = jc_gpu_chunk_data((unsigned char *)buf + offset, len - offset);
		if (csz <= 0 || csz > (len - offset)) {
			return (size_t)-1;
		}
		offset += csz;
...
int main(int argc, char **argv) {
	...
	if (jc_gpu_init() != 0) {
		fprintf(stderr,
				"JC GPU parity requires a loaded GPU kernel. Build PTX first with make -C src/chunking fastcdc_gpu_ptx\n");
		return 2;
	}
	...
	printf("JC parity PASS: cases=%d seed=%u min=%d avg=%d max=%d max_input_len=%d jump_mto=%d mask_bits=%d\n",
			cases, seed, min, avg, max, max_input_len, jump_mto, mask_bits);
