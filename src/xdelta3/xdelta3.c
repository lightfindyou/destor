...
#define CODE_TABLE_VCDIFF_SIZE                \
    (6 * 256) /* Should fit a compressed code \
               * table string */
...
static xd3_output *xd3_alloc_output(xd3_stream *stream, xd3_output *old_output);

static int xd3_alloc_iopt(xd3_stream *stream, usize_t elts);

static void xd3_free_output(xd3_stream *stream, xd3_output *output);

static int xd3_emit_byte(xd3_stream *stream, xd3_output **outputp,
                         uint8_t code);

static int xd3_emit_bytes(xd3_stream *stream, xd3_output **outputp,
                          const uint8_t *base, usize_t size);

static int xd3_emit_double(xd3_stream *stream, xd3_rinst *first,
                           xd3_rinst *second, usize_t code);
static int xd3_emit_single(xd3_stream *stream, xd3_rinst *single, usize_t code);
...
#define BIT_STATE_ENCODE_INIT \
    { 0, 1 }
#define BIT_STATE_DECODE_INIT \
    { 0, 0x100 }
...
const uint16_t __single_hash[256] = {
    ...
    0xbcd1, 0xbb65, 0x42c2, 0xdffe, 0x9666, 0x431b, 0x8504, 0xeb46, 0x6379,
    0xd460, 0xcf14, 0x53cf, 0xdb51, 0xdb08, 0x12c8, 0xf602, 0xe766, 0x2394,
    0x250d, 0xdcbb, 0xa678, 0x02af, 0xa5c6, 0x7ea6, 0xb645, 0xcb4d, 0xc44b,
    0xe5dc, 0x9fe6, 0x5b5c, 0x35f5, 0x701a, 0x220f, 0x6c38, 0x1a56, 0x4ca3,
    0xffc6, 0xb152, 0x8d61, 0x7a58, 0x9025, 0x8b3d, 0xbf0f, 0x95a3, 0xe5f4,
    0xc127, 0x3bed, 0x320b, 0xb7f3, 0x6054, 0x333c, 0xd383, 0x8154, 0x5242,
    0x4e0d, 0x0a94, 0x7028, 0x8689, 0x3a22, 0x0980, 0x1847, 0xb0f1, 0x9b5c,
    0x4176, 0xb858, 0xd542, 0x1f6c, 0x2497, 0x6a5a, 0x9fa9, 0x8c5a, 0x7743,
    0xa8a9, 0x9a02, 0x4918, 0x438c, 0xc388, 0x9e2b, 0x4cad, 0x01b6, 0xab19,
    0xf777, 0x365f, 0x1eb2, 0x091e, 0x7bf8, 0x7a8e, 0x5227, 0xeab1, 0x2074,
    0x4523, 0xe781, 0x01a3, 0x163d, 0x3b2e, 0x287d, 0x5e7f, 0xa063, 0xb134,
    0x8fae, 0x5e8e, 0xb7b7, 0x4548, 0x1f5a, 0xfa56, 0x7a24, 0x900f, 0x42dc,
    0xcc69, 0x02a0, 0x0b22, 0xdb31, 0x71fe, 0x0c7d, 0x1732, 0x1159, 0xcb09,
    0xe1d2, 0x1351, 0x52e9, 0xf536, 0x5a4f, 0xc316, 0x6bf9, 0x8994, 0xb774,
    0x5f3e, 0xf6d6, 0x3a61, 0xf82c, 0xcc22, 0x9d06, 0x299c, 0x09e5, 0x1eec,
    0x514f, 0x8d53, 0xa650, 0x5c6e, 0xc577, 0x7958, 0x71ac, 0x8916, 0x9b4f,
    0x2c09, 0x5211, 0xf6d8, 0xcaaa, 0xf7ef, 0x287f, 0x7a94, 0xab49, 0xfa2c,
    0x7222, 0xe457, 0xd71a, 0x00c3, 0x1a76, 0xe98c, 0xc037, 0x8208, 0x5c2d,
    0xdfda, 0xe5f5, 0x0b45, 0x15ce, 0x8a7e, 0xfcad, 0xaa2d, 0x4b5c, 0xd42e,
    0xb251, 0x907e, 0x9a47, 0xc9a6, 0xd93f, 0x085e, 0x35ce, 0xa153, 0x7e7b,
    0x9f0b, 0x25aa, 0x5d9f, 0xc04d, 0x8a0e, 0x2875, 0x4a1c, 0x295f, 0x1393,
    0xf760, 0x9178, 0x0f5b, 0xfa7d, 0x83b4, 0x2082, 0x721d, 0x6462, 0x0368,
    0x67e2, 0x8624, 0x194d, 0x22f6, 0x78fb, 0x6791, 0xb238, 0xb332, 0x7276,
    0xf272, 0x47ec, 0x4504, 0xa961, 0x9fc8, 0x3fdc, 0xb413, 0x007a, 0x0806,
    0x7458, 0x95c6, 0xccaa, 0x18d6, 0xe2ae, 0x1b06, 0xf3f6, 0x5050, 0xc8e8,
    0xf4ac, 0xc04c, 0xf41c, 0x992f, 0xae44, 0x5f1b, 0x1113, 0x1738, 0xd9a8,
    0x19ea, 0x2d33, 0x9698, 0x2fe9, 0x323f, 0xcde2, 0x6d71, 0xe37d, 0xb697,
    0x2c4f, 0x4373, 0x9102, 0x075d, 0x8e25, 0x1672, 0xec28, 0x6acb, 0x86cc,
...
#define XD3_CHOOSE_INSTRUCTION(stream, prev, inst) \
    xd3_choose_instruction(stream->code_table_desc, prev, inst)
...
struct _xd3_code_table_desc {
    ...
    uint8_t addcopy_add_max;      /* Maximum add size for an add-copy double
                                     instruction,      all modes (default 4) */
    uint8_t addcopy_near_cpy_max; /* Maximum cpy size for an add-copy double
                                     instruction, up through VCD_NEAR modes
                                     (default 6) */
    uint8_t addcopy_same_cpy_max; /* Maximum cpy size for an add-copy double
                                     instruction, VCD_SAME modes (default 4) */

    uint8_t copyadd_add_max;      /* Maximum add size for a copy-add double
                                     instruction,      all modes (default 1) */
    uint8_t copyadd_near_cpy_max; /* Maximum cpy size for a copy-add double
                                     instruction, up through VCD_NEAR modes
                                     (default 4) */
    uint8_t copyadd_same_cpy_max; /* Maximum cpy size for a copy-add double
                                     instruction, VCD_SAME modes (default 4) */
...
static void xd3_build_code_table(const xd3_code_table_desc *desc,
    ...
    for (mode = 0; mode < cpy_modes; mode += 1) {
        ...
        for (size1 = MIN_MATCH; size1 <= max; size1 += 1) {
            for (size2 = 1; size2 <= desc->copyadd_add_max;
                 size2 += 1, d += 1) {
                d->type1 = XD3_CPY + mode;
                d->size1 = size1;
                d->type2 = XD3_ADD;
...
static const xd3_dinst *xd3_alternate_code_table(void) {
 ...
 * sets INST->code1. */
static void xd3_choose_instruction(const xd3_code_table_desc *desc,
                                   xd3_rinst *prev, xd3_rinst *inst) {
    switch (inst->type) {
        case XD3_RUN:
        ...
        default: {
            ...
            if (inst->size < MIN_MATCH + desc->cpy_sizes &&
                ...
                if ((prev != NULL) && (prev->type == XD3_ADD) &&
                    (prev->size <= desc->addcopy_add_max)) {
                    const xd3_code_table_sizes *sizes =
                        &desc->addcopy_max_sizes[mode];
            ...
            if (inst->size <= 18 && inst->size >= 4) {
                ...
                if ((prev != NULL) && (prev->type == XD3_ADD) &&
                    ...
                    if ((inst->size <= 6) && (mode <= 5)) {
                        prev->code2 = 163 + (mode * 12) +
                                      (3 * (prev->size - 1)) + (inst->size - 4);
...
static const uint8_t __alternate_code_table_compressed[178] = {
    ,
};

static int xd3_compute_alternate_table_encoding(xd3_stream *stream,
                                                const uint8_t **data,
                                                usize_t *size) {
...
int xd3_compute_code_table_encoding(xd3_stream *in_stream,
    ...
    return xd3_encode_memory(code_string, CODE_TABLE_STRING_SIZE, dflt_string,
                             CODE_TABLE_STRING_SIZE, comp_string,
                             comp_string_size, CODE_TABLE_VCDIFF_SIZE,
                             /* flags */ 0);
}
...
static int xd3_compute_alternate_table_encoding(xd3_stream *stream,
    ...
    if (__alternate_code_table_compressed[0] == 0) {
        if ((ret = xd3_compute_code_table_encoding(
                 stream, xd3_alternate_code_table(),
...
#if GENERIC_ENCODE_TABLES_COMPUTE_PRINT
        {
            ...
            DP(RINT,
               "\nstatic const usize_t __alternate_code_table_compressed_size "
               "= %u;\n",
               __alternate_code_table_compressed_size);

            DP(RINT,
               "static const uint8_t __alternate_code_table_compressed[%u] "
               "=\n{",
               __alternate_code_table_compressed_size);
    ...
    for (s = 0; s < 6; s += 1) {
        for (i = 0; i < 256; i += 1) {
            switch (s) {
                case 0:
                    *str++ =
                        (code_table[i].type1 >= XD3_CPY ? XD3_CPY
                                                        : code_table[i].type1);
                    break;
                case 1:
                    *str++ =
                        (code_table[i].type2 >= XD3_CPY ? XD3_CPY
                                                        : code_table[i].type2);
                    break;
                case 2:
                    *str++ = (code_table[i].size1);
                    break;
                case 3:
                    *str++ = (code_table[i].size2);
                    break;
                case 4:
                    *str++ = (code_table[i].type1 >= XD3_CPY
                                  ? code_table[i].type1 - XD3_CPY
                                  : 0);
                    break;
                case 5:
                    *str++ = (code_table[i].type2 >= XD3_CPY
                                  ? code_table[i].type2 - XD3_CPY
                                  : 0);
                    break;
            }
        }
...
/* This function translates the code table string into the internal
 * representation.  The stream's near and same-modes should already be set. */
static int xd3_apply_table_string(xd3_stream *stream,
                                  const uint8_t *code_string) {
    int i, s;
    int modes = TOTAL_MODES(stream);
    xd3_dinst *code_table;

    if ((code_table = stream->code_table_alloc = (xd3_dinst *)xd3_alloc(
             stream, (usize_t)sizeof(xd3_dinst), 256)) == NULL) {
        return ENOMEM;
    }
...
static int xd3_apply_table_encoding(xd3_stream *in_stream, const uint8_t *data,
                                    usize_t size) {
    uint8_t dflt_string[CODE_TABLE_STRING_SIZE];
    uint8_t code_string[CODE_TABLE_STRING_SIZE];
    usize_t code_size;
    ...
    if ((ret = xd3_decode_memory(data, size, dflt_string,
                                 CODE_TABLE_STRING_SIZE, code_string,
                                 &code_size, CODE_TABLE_STRING_SIZE, 0))) {
        return ret;
    }
...
static size_t xd3_pow2_roundup(size_t x) {
    size_t i = 1;
    while (x > i) {
        i <<= 1U;
    }
...
#define A32_NMAX                                                           \
    5552 /* NMAX is the largest n such that 255n(n+1)/2 + (n+1)(BASE-1) <= \
            2^32-1 */
...
#if XD3_ENCODER
static inline int xd3_emit_byte(xd3_stream *stream, xd3_output **outputp,
                                uint8_t code) {
...
static inline int xd3_emit_bytes(xd3_stream *stream, xd3_output **outputp,
                                 const uint8_t *base, usize_t size) {
    xd3_output *output = (*outputp);
...
#define IF_SIZEOF32(x) \
    if (num < (1U << (7 * (x)))) return (x);
#define IF_SIZEOF64(x) \
    if (num < (1ULL << (7 * (x)))) return (x);
...
static uint32_t xd3_sizeof_uint64_t(uint64_t num) {
    IF_SIZEOF64(1);
    IF_SIZEOF64(2);
    IF_SIZEOF64(3);
    IF_SIZEOF64(4);
...
static int xd3_alloc_cache(xd3_stream *stream) {
    ...
    if (((stream->acache.s_near > 0) &&
         (stream->acache.near_array = (usize_t *)xd3_alloc(
              stream, stream->acache.s_near, (usize_t)sizeof(usize_t))) ==
             NULL) ||
        ((stream->acache.s_same > 0) &&
         (stream->acache.same_array = (usize_t *)xd3_alloc(
              stream, stream->acache.s_same * 256, (usize_t)sizeof(usize_t))) ==
             NULL)) {
        return ENOMEM;
    }
    ...
    switch (stream->flags & XD3_SEC_TYPE) {
        case 0:
            if (stream->flags & XD3_SEC_NOALL) {
                stream->msg =
                    "XD3_SEC flags require a secondary compressor type";
                return XD3_INTERNAL;
            }
            break;
        case XD3_SEC_FGK:
...
static int xd3_getblk(xd3_stream *stream, xoff_t blkno) {
    ...
    IF_DEBUG2(DP(RINT "[getblk] read source block %" Q
                      "u onblk %u blksize %u\n",
...
static int xd3_iopt_finish_encoding(xd3_stream *stream, xd3_rinst *inst) {
    ...
    switch (inst->type) {
        case XD3_CPY: {
            ...
            if ((ret = xd3_encode_address(
                     stream, addr, inst->pos + stream->taroff, &inst->type))) {
                return ret;
            }

            IF_DEBUG2({
                static int cnt;
                DP(RINT "[iopt copy:%d] pos %" Q "u-%" Q "u addr %" Q "u-%" Q
                        "u size %u\n",
        ...
        case XD3_ADD: {
            if ((ret =
                     xd3_emit_bytes(stream, &DATA_TAIL(stream),
    ...
    if (stream->iout != NULL) {
        if (stream->iout->code2 != 0) {
            ...
            if ((ret = xd3_emit_single(stream, stream->iout,
                                       stream->iout->code1))) {
                return ret;
            }
...
static int xd3_iopt_add_finalize(xd3_stream *stream) {
    ...
    if (stream->iout) {
        if ((ret =
                 xd3_emit_single(stream, stream->iout, stream->iout->code1))) {
            return ret;
        }
...
static int xd3_emit_double(xd3_stream *stream, xd3_rinst *first,
    ...
    IF_DEBUG2(DP(RINT "[emit2]: %u %s (%u) %s (%u) code %u\n", first->pos,
                 xd3_rtype_to_string((xd3_rtype)first->type, 0), first->size,
                 xd3_rtype_to_string((xd3_rtype)second->type, 0), second->size,
                 code));
...
static int xd3_emit_hdr(xd3_stream *stream) {
    ...
    if (stream->current_window == 0) {
        ...
        if (use_gencodetbl) {
            hdr_ind |= VCD_CODETABLE;
        }
        if (use_appheader) {
            hdr_ind |= VCD_APPHEADER;
        }

        if ((ret = xd3_emit_byte(stream, &HDR_TAIL(stream), VCDIFF_MAGIC1)) !=
                0 ||
            (ret = xd3_emit_byte(stream, &HDR_TAIL(stream), VCDIFF_MAGIC2)) !=
                0 ||
            (ret = xd3_emit_byte(stream, &HDR_TAIL(stream), VCDIFF_MAGIC3)) !=
                0 ||
            (ret = xd3_emit_byte(stream, &HDR_TAIL(stream), VCDIFF_VERSION)) !=
                0 ||
...
static int xd3_encode_init(xd3_stream *stream, int full_init) {
    ...
    if (full_init) {
        ...
        if (large_comp) {
            ...
            xd3_size_hashtable(stream, hash_values, &stream->large_hash);
        }
        ...
        if (small_comp) {
            ...
            xd3_size_hashtable(stream, hash_values, &stream->small_hash);
        }
    }
    ...
    switch (stream->enc_state) {
        ...
        enc_flush:
            ...
            if ((stream->buf_leftover != NULL) || (stream->buf_avail != 0) ||
                (stream->avail_in < stream->winsize &&
                 !(stream->flags & XD3_FLUSH))) {
            ...
            IF_DEBUG2(
                DP(RINT "[WINSTART:%" Q "u] input bytes %u offset %" Q "u\n",
                ...
                switch (stream->match_state) {
                    case MATCH_TARGET:
                        ...
                        if (stream->avail_in > 0) {
                             ...
                             * unrestricted. */
                            ret = xd3_source_match_setup(stream,
                                                         stream->match_srcpos);
                            XD3_ASSERT(ret == 0);
        ...
        case ENC_POSTOUT:
             ...
             */
            if ((stream->enc_current = stream->enc_current->next_page) !=
                NULL) {
                goto enc_output;
            }
...
static int xd3_process_stream(int is_encode, xd3_stream *stream,
...
static int xd3_process_memory(int is_encode, int (*func)(xd3_stream *),
                              int close_stream, const uint8_t *input,
                              usize_t input_size, const uint8_t *source,
                              usize_t source_size, uint8_t *output,
                              usize_t *output_size, usize_t output_size_max,
                              int flags) {
    xd3_stream stream;
    xd3_config config;
    xd3_source src;
    ...
    if ((ret =
             xd3_process_stream(is_encode, &stream, func, 1, input, input_size,
...
int xd3_decode_stream(xd3_stream *stream, const uint8_t *input,
                      ...
                      usize_t output_size_max) {
    return xd3_process_stream(0, stream, &xd3_decode_input, 1, input,
                              input_size, output, output_size, output_size_max);
}
...
int xd3_decode_memory(const uint8_t *input, usize_t input_size,
    ...
    return xd3_process_memory(0, &xd3_decode_input, 1, input, input_size,
                              source, source_size, output, output_size,
                              output_size_max, flags);
}
...
int xd3_encode_stream(xd3_stream *stream, const uint8_t *input,
                      usize_t input_size, uint8_t *output, usize_t *output_size,
                      usize_t output_size_max) {
    return xd3_process_stream(1, stream, &xd3_encode_input, 1, input,
                              input_size, output, output_size, output_size_max);
}
...
int xd3_encode_memory(const uint8_t *input, usize_t input_size,
                      ...
                      uint8_t *output, usize_t *output_size,
                      usize_t output_size_max, int flags) {
    return xd3_process_memory(1, &xd3_encode_input, 1, input, input_size,
                              source, source_size, output, output_size,
                              output_size_max, flags);
}
#endif
...
static int xd3_srcwin_setup(xd3_stream *stream) {
    ...
    src->srcbase = stream->match_minaddr;
    src->srclen =
        max((usize_t)length, stream->avail_in + (stream->avail_in >> 2));
...
static int xd3_source_match_setup(xd3_stream *stream, xoff_t srcpos) {
    ...
    if (srcpos < frontier_pos &&
        ...
        IF_DEBUG1(DP(RINT "[match_setup] rejected due to src->max_winsize "
                     ...
                     xd3_source_eof(stream->src), srcpos,
                     stream->src->max_winsize));
        goto bad;
    }
    ...
    if ((srcpos < src->srcbase) ||
        ...
        IF_DEBUG1(DP(RINT "[match_setup] srcpos %" Q "u (tgtpos %" Q "u) "
...
static inline int xd3_forward_match(const uint8_t *s1c, const uint8_t *s2c,
                                    int n) {
    int i = 0;
#if UNALIGNED_OK
...
static int xd3_source_extend_match(xd3_stream *stream) {
    ...
    if (stream->match_state == MATCH_BACKWARD) {
        ...
        while (stream->match_back < stream->match_maxback) {
            ...
            IF_DEBUG2(DP(RINT "[maxback] maxback %u trysrc %" Q
                              "u/%u tgt %u tryrem %u\n",
                         stream->match_maxback, tryblk, tryoff, streamoff,
                         tryrem));
    ...
    while (stream->match_fwd < stream->match_maxfwd) {
        ...
        tryrem =
            min(stream->match_maxfwd - stream->match_fwd, src->onblk - tryoff);
    ...
    if (stream->match_fwd < stream->min_match) {
        ...
        IF_DEBUG2({
            static int x = 0;
            DP(RINT "[source match:%d] <inp %" Q "u %" Q "u>  <src %" Q "u %" Q
                    "u> (%s) [ %u bytes ]\n",
               x++, stream->total_in + target_position,
               stream->total_in + target_position + match_length,
               match_position, match_position + match_length,
...
again:
    ...
    while (--chain != 0) {
        ...
        usize_t prev_pos =
            stream->small_prev[base & stream->sprevmask].last_pos;
        usize_t diff_pos;
...
static int xd3_srcwin_move_point(xd3_stream *stream, usize_t *next_move_point) {
    ...
    logical_input_cksum_pos =
        min((stream->total_in + stream->input_position) * 2,
    ...
    while (stream->srcwin_cksum_pos < logical_input_cksum_pos &&
        ...
        IF_DEBUG1(DP(RINT "[srcwin_move_point] T=%" Q "u{%" Q "u} S=%" Q
                          "u EOF=%" Q "u %s\n",
                     stream->total_in + stream->input_position,
                     logical_input_cksum_pos, stream->srcwin_cksum_pos,
                     xd3_source_eof(stream->src),
    ...
    IF_DEBUG1(DP(RINT "[srcwin_move_point] exited loop T=%" Q "u{%" Q "u} "
                      "S=%" Q "u EOF=%" Q "u %s\n",
                 stream->total_in + stream->input_position,
                 logical_input_cksum_pos, stream->srcwin_cksum_pos,
                 xd3_source_eof(stream->src),
    ...
    if (stream->src->eof_known) {
        ...
        if (stream->srcwin_cksum_pos >= source_size) {
            ...
            *next_move_point = USIZE_T_MAX;
            IF_DEBUG1(
                DP(RINT "[srcwin_move_point] finished with source input\n"));
            return 0;
        }
    }
...
restartloop:
    ...
    if (xd3_iopt_last_matched(stream) > stream->input_position) {
        stream->min_match = max(MIN_MATCH, 1 + xd3_iopt_last_matched(stream) -
                                               stream->input_position);
    } else {
        stream->min_match = MIN_MATCH;
    }
    ...
    for (;; inp += 1, stream->input_position += 1) {
        ...
        if (DO_RUN && run_l == SLOOK) {
            ...
            if (run_l >= stream->min_match && run_l >= MIN_RUN) {
                if ((ret = xd3_emit_run(stream, stream->input_position, run_l,
                                        &run_c))) {
                    return ret;
                }
        ...
        if (DO_SMALL) {
            ...
            if (stream->small_table[sinx] != 0) {
                match_length = xd3_smatch(stream, stream->small_table[sinx],
                                          scksum, &match_offset);
            } else {
                match_length = 0;
            }
            ...
            if (match_length >= stream->min_match) {
                IF_DEBUG2({
                    static int x = 0;
                    DP(RINT
                       "[target match:%d] <inp %u %u>  <cpy %u %u> "
                       "(-%d) [ %u bytes ]\n",
                       x++, stream->input_position,
                       stream->input_position + match_length, match_offset,
...
int xdelta3_compress(char *InFile, int InSize, char *SrcFile, int SrcSize,
    ...
    if (BufSize < XD3_ALLOCSIZE) BufSize = XD3_ALLOCSIZE;
    ...
    do {
        ...
        xd3_avail_input(&stream, (const unsigned char *)Input_Buf,
                        Input_Buf_Read);
    ...
    process:
        ...
        switch (ret) {
            ...
            case XD3_GETSRCBLK: {
                source.onblk = min(source.blksize, SrcSize);
                memcpy((void *)source.curblk,
                       SrcFile + source.blksize * source.getblkno,
                       source.onblk);
                SrcSize -= source.onblk;
