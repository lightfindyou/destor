...
void NGT::Command::create(Args &args) {
    const string usage =
        "Usage: ngt create "
        "-d dimension [-p #-of-thread] [-i index-type(t|g)] [-g "
        "graph-type(a|k|b|o|i)] "
        "[-t truncation-edge-limit] [-E edge-size] [-S edge-size-for-search] "
        "[-L edge-size-limit] "
        "[-e epsilon] [-o object-type(f|c)] [-D "
        "distance-function(1|2|a|A|h|j|c|C)] [-n #-of-inserted-objects] "
        "[-P path-adjustment-interval] [-B dynamic-edge-size-base] [-A "
        "object-alignment(t|f)] "
    ...
    if (property.dimension <= 0) {
        cerr << "ngt: Error: Specify greater than 0 for # of your data "
                "dimension by a parameter -d."
             << endl;
        cerr << usage << endl;
        return;
    }

    property.objectAlignment = args.getChar("A", 'f') == 't'
                                   ? NGT::Property::ObjectAlignmentTrue
    ...
    if (property.graphType == NGT::Property::GraphType::GraphTypeONNG) {
        ...
        if (str != "-") {
            ...
            if (str != "-" && tokens.size() != 2) {
                cerr << "ngt: Error: outgoing/incoming edge size specification "
                        "is invalid. (out)x(in) "
                     << str << endl;
                cerr << usage << endl;
                return;
            ...
            cerr << "ngt: ONNG out x in=" << property.outgoingEdge << "x"
                 << property.incomingEdge << endl;
        }
    }
    ...
    switch (distanceType) {
        ...
        case 'j':
            property.distanceType =
                NGT::Index::Property::DistanceType::DistanceTypeJaccard;
            break;
        case 'J':
            property.distanceType =
                NGT::Index::Property::DistanceType::DistanceTypeSparseJaccard;
            break;
        case 'c':
            property.distanceType =
            ...
            break;
        case 'C':
            property.distanceType = NGT::Index::Property::DistanceType::
                DistanceTypeNormalizedCosine;
            break;
        default:
            cerr << "ngt: Error: Invalid distance type. " << distanceType
                 << endl;
            cerr << usage << endl;
            return;
    }
    ...
    if (maxNoOfObjects > 0) {
        property.graphSharedMemorySize = property.treeSharedMemorySize =
            property.objectSharedMemorySize =
...
void NGT::Command::append(Args &args) {
    const string usage =
        "Usage: ngt append [-p #-of-thread] [-d dimension] [-n data-size] "
        "index(output) [data.tsv(input)]";
    string database;
    try {
        database = args.get("#1");
    ...
    try {
        ...
        cerr << "ngt: Warning: No specified object file. Just build an index "
...
void NGT::Command::search(NGT::Index &index,
                          NGT::Command::SearchParameter &searchParameter,
                          istream &is, ostream &stream) {
    ...
    while (getline(is, line)) {
        if (searchParameter.querySize > 0 &&
            queryCount >= searchParameter.querySize) {
            break;
        }
        NGT::Object *object = index.allocateObject(line, " \t");
        queryCount++;
        size_t step =
            searchParameter.step == 0 ? UINT_MAX : searchParameter.step;
        for (size_t n = 0; n <= step; n++) {
            NGT::SearchContainer sc(*object);
            double epsilon;
            if (searchParameter.step != 0) {
                epsilon = searchParameter.beginOfEpsilon +
                          (searchParameter.endOfEpsilon -
                           searchParameter.beginOfEpsilon) *
                              n / step;
            } else {
                epsilon = searchParameter.beginOfEpsilon +
                          searchParameter.stepOfEpsilon * n;
            ...
            try {
                if (searchParameter.outputMode[0] == 'e') {
                    ...
                    size_t trial =
                        searchParameter.trial <= 0 ? 1 : searchParameter.trial;
            ...
            if (searchParameter.outputMode[0] == 'e') {
                ...
                stream << "# Distance Computation="
                       << sc.distanceComputationCount << endl;
            ...
            if (searchParameter.outputMode[0] == 'e') {
                ...
                stream << "Query Time= " << timer.time << " (sec), "
    ...
    if (searchParameter.outputMode[0] == 'e') {
        stream << "# Average Query Time (msec)="
               << totalTime * 1000.0 / (double)queryCount << endl;
        ...
        if (searchParameter.outputMode == "e+") {
            ...
            for (size_t id = 1; id < graph.repository.size(); id++) {
                ...
                try {
                    ...
                    cerr << "Graph::search: Warning. Cannot get the node. ID="
                         << id << ":" << err.what()
...
#if defined(NGT_SHARED_MEMORY_ALLOCATOR)
                    distance +=
                        (*node).at(i, graph.repository.allocator).distance;
#else
            ...
            stream << "# Average number of edges="
                   << (double)numberOfEdges / (double)numberOfNodes << endl;
            stream << "# Average distance of edges=" << setprecision(10)
                   << distance / (double)numberOfEdges << endl;
        }
    } else {
        stream << "Average Query Time= " << totalTime / (double)queryCount
               << " (sec), " << totalTime * 1000.0 / (double)queryCount
               << " (msec), (" << totalTime << "/" << queryCount << ")" << endl;
    }
}

void NGT::Command::search(Args &args) {
    const string usage =
        "Usage: ngt search [-i index-type(g|t|s)] [-n result-size] [-e "
        "epsilon] [-E edge-size] "
    ...
    if (debugLevel >= 1) {
        ...
        cerr << "epsilon=" << searchParameter.beginOfEpsilon << "<->"
             << searchParameter.endOfEpsilon << ","
...
void NGT::Command::remove(Args &args) {
    const string usage =
        "Usage: ngt remove [-d object-ID-type(f|d)] [-m f] index(input) "
        "object-ID(input)";
    string database;
    try {
        database = args.get("#1");
    ...
    try {
        ...
        if (dataType == 'f') {
            ...
            ifstream is(ids);
            if (!is) {
                cerr << "ngt: Error: Cannot open the specified file. " << ids
                     << endl;
                cerr << usage << endl;
                return;
            }
...
void NGT::Command::prune(Args &args) {
    const string usage =
        "Usage: ngt prune -e #-of-forcedly-pruned-edges -s "
        "#-of-selecively-pruned-edge index(in/out)";
    string indexName;
    try {
    ...
    cerr << "selectively pruned edge size=" << selectivelyPrunedEdgeSize
         << endl;

    if (selectivelyPrunedEdgeSize == 0 && forcedlyPrunedEdgeSize == 0) {
        cerr << "prune: Error! Either of selective edge size or remaining edge "
    ...
    if (forcedlyPrunedEdgeSize != 0 && selectivelyPrunedEdgeSize != 0 &&
        ...
        cerr << "prune: Error! selective edge size is less than remaining edge "
                "size."
             << endl;
        cerr << usage << endl;
        return;
    ...
    for (size_t id = 1; id < graph.repository.size(); id++) {
        try {
            ...
            if (forcedlyPrunedEdgeSize > 0 &&
                node.size() >= forcedlyPrunedEdgeSize) {
#ifdef NGT_SHARED_MEMORY_ALLOCATOR
            ...
            if (selectivelyPrunedEdgeSize > 0 &&
                node.size() >= selectivelyPrunedEdgeSize) {
#ifdef NGT_SHARED_MEMORY_ALLOCATOR
                cerr << "not implemented" << endl;
                abort();
#else
                size_t rank = 0;
                for (NGT::GraphNode::iterator i = node.begin(); i != node.end();
                     ++rank) {
                    if (rank >= selectivelyPrunedEdgeSize) {
                        bool found = false;
                        for (size_t t1 = 0; t1 < node.size() && found == false;
                             ++t1) {
            ...
            cerr << "Graph::search: Warning. Cannot get the node. ID=" << id
                 << ":" << err.what() << endl;
            continue;
        }
    }
...
void NGT::Command::reconstructGraph(Args &args) {
    const string usage =
        "Usage: ngt reconstruct-graph [-m mode] [-P path-adjustment-mode] -o "
        "#-of-outgoing-edges -i #-of-incoming(reversed)-edges [-q "
        "#-of-queries] [-n #-of-results] index(input) index(output)\n"
        "\t-m mode\n"
        ...
        "\t\ta: Advanced method. High-speed. Not guarantee the paper's method. "
        "(default)\n"
        "\t\tothers: Slow and less memory usage, but guarantee the paper's "
        "method.\n";
    ...
    if (mode == 'P') {
        ...
        std::cerr << "ngt::reconstructGraph: Warning. \'-m P\' and not zero "
                     "for # of in/out edges are specified at the same time."
                  << std::endl;
    }
    graphOptimizer.shortcutReduction =
        (mode == 'S' || mode == 'C' || mode == 'P') ? true : false;
    graphOptimizer.searchParameterOptimization =
        (smode == '-' || smode == 's') ? true : false;
    graphOptimizer.prefetchParameterOptimization =
        (smode == '-' || smode == 'p') ? true : false;
    graphOptimizer.accuracyTableGeneration =
        (smode == '-' || smode == 'a') ? true : false;
    graphOptimizer.margin = margin;
    graphOptimizer.gtEpsilon = gtEpsilon;

    graphOptimizer.set(numOfOutgoingEdges, numOfIncomingEdges, nOfQueries,
                       nOfResults);
...
void NGT::Command::optimizeSearchParameters(Args &args) {
    const string usage =
        "Usage: ngt optimize-search-parameters [-m optimization-target(s|p|a)] "
        "[-q #-of-queries] [-n #-of-results] index\n"
        "\t-m mode\n"
        "\t\ts: optimize search parameters (the number of explored edges).\n"
        "\t\tp: optimize prefetch prameters.\n"
        "\t\ta: generate an accuracy table to specify an expected accuracy "
        "instead of an epsilon for search.\n";
    ...
    try {
        ...
        graphOptimizer.searchParameterOptimization =
            (mode == '-' || mode == 's') ? true : false;
        graphOptimizer.prefetchParameterOptimization =
            (mode == '-' || mode == 'p') ? true : false;
        graphOptimizer.accuracyTableGeneration =
            (mode == '-' || mode == 'a') ? true : false;
...
void NGT::Command::refineANNG(Args &args) {
...
#else
    const string usage =
        "Usage: ngt refine-anng [-e epsilon] [-a expected-accuracy] anng-index "
        "refined-anng-index";
    ...
    try {
        GraphReconstructor::refineANNG(index, epsilon, expectedAccuracy,
                                       noOfEdges, exploreEdgeSize, batchSize);
    } catch (NGT::Exception &err) {
        std::cerr << "Error!! Cannot refine the index. " << err.what()
                  << std::endl;
        return;
    }
...
void NGT::Command::repair(Args &args) {
    ...
    if (mode == 'r') {
        ...
        if (stat != 0) {
            std::cerr << "ngt::repair: Cannot create the specified index. "
                      << path << std::endl;
            cerr << usage << endl;
            return;
        }
    ...
    NGT::ObjectRepository &objectRepository =
        index.getObjectSpace().getRepository();
    NGT::GraphIndex &graphIndex = static_cast<GraphIndex &>(index.getIndex());
    NGT::GraphAndTreeIndex &graphAndTreeIndex =
        static_cast<GraphAndTreeIndex &>(index.getIndex());
    ...
    if (objSize < idsSize) {
        std::cerr << "The sizes of the repository and tree are inconsistent. "
                  << objSize << ":" << idsSize << std::endl;
    }
        ...
        if (ids.find(id) != ids.end()) {
            if (removedIDs.find(id) != removedIDs.end() || id >= objSize) {
                if (repair) {
                    ...
                    std::cerr << "Found the removed object in the tree. "
                                 "Removed it from the tree. "
                              << id << std::endl;
                } else {
                    std::cerr << "Found the removed object in the tree. " << id
                              << std::endl;
                }
            ...
            if (removedIDs.find(id) == removedIDs.end() && id < objSize) {
                std::cerr << "Not found an object in the tree. However, it "
                             "might be a duplicated object. "
                          << id << std::endl;
    ...
    if (objSize != graphIndex.repository.size()) {
        std::cerr << "The sizes of the repository and graph are inconsistent. "
                  << objSize << ":" << graphIndex.repository.size()
                  << std::endl;
    }
    size_t invalidGraphObjectCount = 0;
    size_t uninsertedGraphObjectCount = 0;
    size = objSize > graphIndex.repository.size()
               ? objSize
               : graphIndex.repository.size();
        ...
        try {
            ...
            if (removedIDs.find(id) != removedIDs.end() || id >= objSize) {
                if (repair) {
                    ...
                    std::cerr << "Found the removed object in the graph. "
                                 "Removed it from the graph. "
                              << id << std::endl;
                } else {
                    std::cerr << "Found the removed object in the graph. " << id
                              << std::endl;
                }
            ...
            if (removedIDs.find(id) == removedIDs.end() && id < objSize) {
                std::cerr << "Not found an object in the graph. It should be "
...
#endif
                if (removedIDs.find((*n).id) != removedIDs.end() ||
                    (*n).id >= objSize) {
                    std::cerr
                        << "Not found the destination object of the edge. "
                        << id << ":" << (*n).id << std::endl;
                    invalidEdgeCount++;
                    if (repair) {
#if defined(NGT_SHARED_MEMORY_ALLOCATOR)
    ...
    std::cerr << "The number of invalid tree objects=" << invalidTreeObjectCount
              << std::endl;
    std::cerr << "The number of invalid graph objects="
              << invalidGraphObjectCount << std::endl;
    std::cerr << "The number of uninserted tree objects (Can be ignored)="
              << uninsertedTreeObjectCount << std::endl;
    std::cerr << "The number of uninserted graph objects="
              << uninsertedGraphObjectCount << std::endl;
    std::cerr << "The number of invalid edges=" << invalidEdgeCount
              << std::endl;
...
void NGT::Command::optimizeNumberOfEdgesForANNG(Args &args) {
    const string usage =
        "Usage: ngt optimize-#-of-edges [-q #-of-queries] [-k "
        "#-of-retrieved-objects] "
        "[-p #-of-threads] [-a target-accuracy] [-o target-#-of-objects] [-s "
        "#-of-sampe-objects] "
    ...
    parameter.targetNoOfObjects =
        args.getl("o", 0);  // zero will replaced # of the repository size.
    ...
    auto optimizedEdge =
        graphOptimizer.optimizeNumberOfEdgesForANNG(indexPath, parameter);
    std::cout << "The optimized # of edges=" << optimizedEdge.first << "("
              << optimizedEdge.second << ")" << std::endl;
    std::cout << "Successfully completed." << std::endl;
}
...
void NGT::Command::info(Args &args) {
    ...
    try {
        ...
        NGT::GraphIndex::showStatisticsOfGraph(
            static_cast<NGT::GraphIndex &>(index.getIndex()), mode, edgeSize);
        if (mode == 'v') {
