...
class ThreadTerminationException : public Exception {
   public:
    ThreadTerminationException(const std::string &file, size_t line,
                               std::stringstream &m) {
        set(file, line, m.str());
    }
    ThreadTerminationException(const std::string &file, size_t line,
   ...
   public:
    class JobQueue : public std::deque<JOB> {
       public:
        ...
        ~JobQueue() { Thread::destructThreadMutex(threadMutex); }
       ...
       public:
        ...
        void popFront(JOB &d) {
            ...
            while (JobQueue::isEmpty()) {
                if (isTerminate) {
                    JobQueue::unlock();
                    NGTThrowSpecificException("Thread::termination",
        ...
        void pushBackEnd() { underPushing = false; }
       ...
       public:
        SHARED_DATA &getSharedData() {
            if (threadPool->sharedData.isAvailable) {
                ...
                NGTThrowException(
                    "Thread::getSharedData: Shared data is unavailable. No set "
                    "yet.");
            }
        }
