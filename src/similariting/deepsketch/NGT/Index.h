...
class Index {
   public:
        ...
        void exportProperty(NGT::PropertySet &p) {
            ...
            switch (distanceType) {
                ...
                case DistanceType::DistanceTypeNormalizedCosine:
                    ...
                    break;
                default:
                    std::cerr << "Fatal error. Invalid distance type. "
                              << distanceType << std::endl;
                    abort();
            }
            switch (indexType) {
                ...
                case IndexType::Graph:
                    ...
                    break;
                default:
                    std::cerr << "Fatal error. Invalid index type. "
                              << indexType << std::endl;
                    abort();
            }
            switch (databaseType) {
                ...
                case DatabaseType::MemoryMappedFile:
                    ...
                    break;
                default:
                    std::cerr << "Fatal error. Invalid database type. "
            ...
            switch (objectAlignment) {
                ...
                case ObjectAlignment::ObjectAlignmentFalse:
                    ...
                    break;
                default:
                    std::cerr << "Fatal error. Invalid objectAlignment. "
                              << objectAlignment << std::endl;
                    abort();
            }
        ...
        void importProperty(NGT::PropertySet &p) {
            ...
            if (it != p.end()) {
                if (it->second == "Float-4") {
                    ...
                    std::cerr << "Invalid Object Type in the property. "
            ...
            if (it != p.end()) {
                if (it->second == "None") {
                    ...
                    std::cerr << "Invalid Distance Type in the property. "
                              << it->first << ":" << it->second << std::endl;
                }
            } else {
            ...
            if (it != p.end()) {
                if (it->second == "GraphAndTree") {
                    ...
                    std::cerr << "Invalid Index Type in the property. "
                              << it->first << ":" << it->second << std::endl;
                }
            } else {
            ...
            if (it != p.end()) {
                if (it->second == "Memory") {
                    ...
                    std::cerr << "Invalid Database Type in the property. "
                              << it->first << ":" << it->second << std::endl;
                }
            } else {
            ...
            if (it != p.end()) {
                if (it->second == "None") {
                    ...
                    std::cerr << "Invalid Object Alignment in the property. "
                              << it->first << ":" << it->second << std::endl;
                }
            } else {
            ...
            pathAdjustmentInterval =
                p.getl("PathAdjustmentInterval", pathAdjustmentInterval);
#ifdef NGT_SHARED_MEMORY_ALLOCATOR
            graphSharedMemorySize =
                p.getl("GraphSharedMemorySize", graphSharedMemorySize);
            treeSharedMemorySize =
                p.getl("TreeSharedMemorySize", treeSharedMemorySize);
            objectSharedMemorySize =
                p.getl("ObjectSharedMemorySize", objectSharedMemorySize);
#endif
    ...
    class InsertionResult {
       public:
        ...
        InsertionResult(size_t i, bool tf, Distance d)
            : id(i), identical(tf), distance(d) {}
        size_t id;
        bool identical;
        Distance distance;  // the distance between the centroid and the
                            // inserted object.
    };
    ...
    class AccuracyTable {
       public:
        ...
        void set(std::string str) {
            ...
            for (auto i = tokens.begin(); i != tokens.end(); ++i) {
                ...
                if (ts.size() != 2) {
                    ...
                    msg << "AccuracyTable: Invalid accuracy table string " << *i
                        << ":" << str;
                    NGTThrowException(msg);
                }
                table.push_back(std::make_pair(Common::strtod(ts[0]),
                                               Common::strtod(ts[1])));
            }
        }
        ...
        float getEpsilon(double accuracy) {
            if (table.size() <= 2) {
                std::stringstream msg;
                msg << "AccuracyTable: The accuracy table is not set yet. The "
                       "table size="
                    << table.size();
                NGTThrowException(msg);
            }
            ...
            float e = lower.first + (upper.first - lower.first) *
                                        (accuracy - lower.second) /
...
#endif
    Index(const std::string &database, bool rdOnly = false) : index(0) {
        open(database, rdOnly);
    }
    Index(const std::string &database, NGT::Property &prop) : index(0) {
        open(database, prop);
    }
    virtual ~Index() { close(); }
    ...
    static void mkdir(const std::string &dir) {
        if (::mkdir(dir.c_str(),
            ...
            std::stringstream msg;
            msg << "NGT::Index::mkdir: Cannot make the specified directory. "
                << dir;
            NGTThrowException(msg);
        }
    }
    static void create(const std::string &database, NGT::Property &prop,
                       bool redirect = false) {
        createGraphAndTree(database, prop, redirect);
    }
    static void createGraphAndTree(const std::string &database,
                                   NGT::Property &prop,
                                   const std::string &dataFile,
                                   size_t dataSize = 0, bool redirect = false);
    static void createGraphAndTree(const std::string &database,
                                   NGT::Property &prop, bool redirect = false) {
        createGraphAndTree(database, prop, "", redirect);
    }
    static void createGraph(const std::string &database, NGT::Property &prop,
                            const std::string &dataFile, size_t dataSize = 0,
                            bool redirect = false);
    template <typename T>
    size_t insert(const std::vector<T> &object);
    template <typename T>
    size_t append(const std::vector<T> &object);
    static void append(const std::string &database, const std::string &dataFile,
                       size_t threadSize, size_t dataSize);
    static void append(const std::string &database, const float *data,
                       size_t dataSize, size_t threadSize);
    static void remove(const std::string &database,
                       std::vector<ObjectID> &objects, bool force = false);
    static void exportIndex(const std::string &database,
                            const std::string &file);
    static void importIndex(const std::string &database,
                            const std::string &file);
    virtual void load(const std::string &ifile, size_t dataSize) {
        getIndex().load(ifile, dataSize);
    }
    virtual void append(const std::string &ifile, size_t dataSize) {
        getIndex().append(ifile, dataSize);
    }
    ...
    virtual size_t getObjectRepositorySize() {
        return getIndex().getObjectRepositorySize();
    }
    ...
    virtual void saveIndex(const std::string &ofile) {
        getIndex().saveIndex(ofile);
    }
    virtual void loadIndex(const std::string &ofile) {
        getIndex().loadIndex(ofile);
    }
    virtual Object *allocateObject(const std::string &textLine,
                                   const std::string &sep) {
        return getIndex().allocateObject(textLine, sep);
    }
    virtual Object *allocateObject(const std::vector<double> &obj) {
        return getIndex().allocateObject(obj);
    }
    virtual Object *allocateObject(const std::vector<float> &obj) {
        return getIndex().allocateObject(obj);
    }
    virtual Object *allocateObject(const std::vector<uint8_t> &obj) {
        return getIndex().allocateObject(obj);
    }
    ...
    virtual void setProperty(NGT::Property &prop) {
        getIndex().setProperty(prop);
    }
    virtual void getProperty(NGT::Property &prop) {
        getIndex().getProperty(prop);
    }
    virtual void deleteObject(Object *po) { getIndex().deleteObject(po); }
    virtual void linearSearch(NGT::SearchContainer &sc) {
        getIndex().linearSearch(sc);
    }
    virtual void linearSearch(NGT::SearchQuery &sc) {
        getIndex().linearSearch(sc);
    }
    ...
    virtual void exportIndex(const std::string &file) {
        getIndex().exportIndex(file);
    }
    virtual void importIndex(const std::string &file) {
        getIndex().importIndex(file);
    }
    virtual bool verify(std::vector<uint8_t> &status, bool info = false,
                        char mode = '-') {
        return getIndex().verify(status, info, mode);
    }
    virtual ObjectSpace &getObjectSpace() {
        return getIndex().getObjectSpace();
    }
    virtual size_t getSharedMemorySize(
        std::ostream &os, SharedMemoryAllocator::GetMemorySizeType t =
                              SharedMemoryAllocator::GetTotalMemorySize) {
#ifdef NGT_SHARED_MEMORY_ALLOCATOR
        size_t osize =
            getObjectSpace().getRepository().getAllocator().getMemorySize(t);
#else
        size_t osize = 0;
#endif
   ...
   protected:
    ...
    static void loadAndCreateIndex(Index &index, const std::string &database,
                                   const std::string &dataFile,
                                   size_t threadSize, size_t dataSize);
...
class GraphIndex : public Index, public NeighborhoodGraph {
   public:
    ...
    GraphIndex(const std::string &allocator, NGT::Property &prop)
        : readOnly(false) {
    ...
    void destructObjectSpace() {
        ...
        if (property.objectType == NGT::ObjectSpace::ObjectType::Float) {
            ObjectSpaceRepository<float, double> *os =
                (ObjectSpaceRepository<float, double> *)objectSpace;
#ifndef NGT_SHARED_MEMORY_ALLOCATOR
            os->deleteAll();
#endif
            delete os;
        } else if (property.objectType == NGT::ObjectSpace::ObjectType::Uint8) {
            ObjectSpaceRepository<unsigned char, int> *os =
                (ObjectSpaceRepository<unsigned char, int> *)objectSpace;
#ifndef NGT_SHARED_MEMORY_ALLOCATOR
            os->deleteAll();
#endif
            delete os;
        } else {
            std::cerr << "Cannot find Object Type in the property. "
                      << property.objectType << std::endl;
            return;
        }
        objectSpace = 0;
    }
    ...
    void saveObjectRepository(const std::string &ofile) {
        ...
        if (objectSpace != 0) {
            ...
            std::cerr
                << "saveIndex::Warning! ObjectSpace is null. continue saving..."
                << std::endl;
        }
#endif
    }
    ...
    void getRandomSeeds(REPOSITORY &repo, ObjectDistances &seeds,
                        size_t seedSize) {
        // clear all distances to find the same object as a randomized object.
        for (ObjectDistances::iterator i = seeds.begin(); i != seeds.end();
             i++) {
            (*i).distance = 0.0;
        }
        ...
        repositorySize =
            repositorySize == 0
                ? 0
                : repositorySize -
                      1;  // Because the head of repository is a dummy.
    ...
    void remove(const ObjectID id, bool force) {
        ...
        try {
            ...
            std::cerr
                << "NGT::GraphIndex::remove:: cannot remove from feature. id="
    ...
    virtual void searchForNNGInsertion(Object &po, ObjectDistances &result) {
        ...
        sc.explorationCoefficient =
            NeighborhoodGraph::property.insertionRadiusCoefficient;
        try {
    ...
    void searchForKNNGInsertion(Object &po, ObjectID id,
        ...
        if ((size_t)NeighborhoodGraph::property.edgeSizeForCreation !=
            result.size()) {
            std::cerr << "searchForKNNGInsert::Warning! inconsistency of the "
                         "sizes. ID="
                      << id << " "
                      << NeighborhoodGraph::property.edgeSizeForCreation << ":"
    ...
    void checkGraph() {
        ...
        for (size_t id = 0; id < fr.size(); id++) {
            ...
            if (rs.size() != objects->size()) {
                std::cerr << "Cannot get the specified number of the results. "
                          << rs.size() << ":" << objects->size() << std::endl;
            }
            size_t count = 0;
            ...
            if (count != objects->size()) {
                std::cerr << "id=" << id << " identities=" << count << " "
                          << objects->size() << " " << rs.size() << std::endl;
            }
        }
    }
    ...
    virtual bool verify(std::vector<uint8_t> &status, bool info) {
        ...
        if (repo.size() != fr.size()) {
            if (info) {
                std::cerr
                    << "Warning! # of nodes is different from # of objects. "
                    << repo.size() << ":" << fr.size() << std::endl;
            }
        }
        status.clear();
        ...
        for (size_t id = 1; id < fr.size(); id++) {
            if (fr[id] == 0) {
                if (id < repo.size() && repo[id] != 0) {
                    std::cerr << "Error! The node exists in the graph, but the "
            ...
            if (fr[id] != 0 && repo[id] == 0) {
                std::cerr << "Error. No." << id
                          << " is not registerd in the graph." << std::endl;
                valid = false;
            }
            if ((id % 1000000) == 0) {
...
#endif
                    if (po == 0) {
                        std::cerr << "Error! Cannot get the object. " << id
                                  << std::endl;
                        valid = false;
                        continue;
                    }
            ...
            if (id >= repo.size()) {
                std::cerr << "Error. No." << id
                          << " is not registerd in the object repository. "
                          << repo.size() << std::endl;
                valid = false;
            }
            if (id < repo.size() && repo[id] != 0) {
                try {
                    GraphNode *objects = getNode(id);
                    if (objects == 0) {
                        std::cerr << "Error! Cannot get the node. " << id
                                  << std::endl;
                        valid = false;
                    }
#if defined(NGT_SHARED_MEMORY_ALLOCATOR)
                    for (GraphNode::iterator ri =
                             objects->begin(repo.allocator);
                    ...
                    for (GraphNode::iterator ri = objects->begin();
                        ...
                        for (GraphNode::iterator rj =
                                 objects->begin(repo.allocator) +
                                 std::distance(objects->begin(repo.allocator),
                                               ri);
                            ...
                            if ((*ri).id == (*rj).id &&
                                std::distance(objects->begin(repo.allocator),
                                              ri) !=
                                    std::distance(
                                        objects->begin(repo.allocator), rj)) {
                                std::cerr
                                    << "Error! More than two identical "
                                       "objects! ID="
                                    << (*rj).id << " idx="
                                    << std::distance(
                                           objects->begin(repo.allocator), ri)
                                    << ":"
                                    << std::distance(
                                           objects->begin(repo.allocator), rj)
                                    << " disntace=" << (*ri).distance << ":"
                        ...
                        for (GraphNode::iterator rj =
                            ...
                            if ((*ri).id == (*rj).id &&
                                ...
                                std::cerr << "Error! More than two identical "
                                             "objects! ID="
                                          << (*rj).id << " idx="
                                          << std::distance(objects->begin(), ri)
                                          << ":"
                        ...
                        if ((*ri).id == 0 || (*ri).id >= repo.size()) {
                            std::cerr << "Error! Neighbor's ID of the node is "
                                         "out of range. ID="
                                      << id << std::endl;
                            valid = false;
                        } else if (repo[(*ri).id] == 0) {
                            std::cerr << "Error! The neighbor ID of the node "
                                         "is invalid. ID="
                                      << id << " Invalid ID=" << (*ri).id
                                      << std::endl;
                            if (fr[(*ri).id] == 0) {
                                std::cerr
                                    << "The neighbor doesn't exist in the "
                                       "object repository as well. ID="
                                    << (*ri).id << std::endl;
                            } else {
                                std::cerr << "The neighbor exists in the "
                        ...
                        if ((*ri).distance < 0.0) {
                            std::cerr
                                << "Error! Neighbor's distance is munus. ID="
                                << id << std::endl;
                            valid = false;
                        }
                    }
    ...
    static bool showStatisticsOfGraph(NGT::GraphIndex &outGraph,
                                      char mode = '-',
                                      size_t edgeSize = UINT_MAX);

    size_t getObjectRepositorySize() {
        return objectSpace->getRepository().size();
    }

    size_t getSizeOfElement() { return objectSpace->getSizeOfElement(); }

    Object *allocateObject(const std::string &textLine,
                           const std::string &sep) {
    ...
    NeighborhoodGraph::Property &getGraphProperty() {
        return NeighborhoodGraph::property;
    }
    ...
    float getEpsilonFromExpectedAccuracy(double accuracy) {
        return accuracyTable.getEpsilon(accuracy);
    }
   ...
   protected:
    ...
    void getSeedsFromGraph(REPOSITORY &repo, ObjectDistances &seeds) {
        if (repo.size() != 0) {
            ...
            if (NeighborhoodGraph::property.seedType ==
                    NeighborhoodGraph::SeedTypeRandomNodes ||
                NeighborhoodGraph::property.seedType ==
                    NeighborhoodGraph::SeedTypeNone) {
                getRandomSeeds(repo, seeds, seedSize);
            } else if (NeighborhoodGraph::property.seedType ==
                       NeighborhoodGraph::SeedTypeFixedNodes) {
                ...
                for (size_t i = 1; i <= seedSize; i++) {
                    ...
                    seeds.push_back(obj);
                       ...
                       NeighborhoodGraph::SeedTypeFirstNode) {
    ...
    void (*searchUnupdatableGraph)(NGT::NeighborhoodGraph &,
                                   NGT::SearchContainer &,
                                   NGT::ObjectDistances &);
#endif
...
class GraphAndTreeIndex : public GraphIndex, public DVPTree {
   public:
#ifdef NGT_SHARED_MEMORY_ALLOCATOR
    GraphAndTreeIndex(const std::string &allocator, bool rdOnly = false)
        : GraphIndex(allocator, false) {
...
#else
    GraphAndTreeIndex(const std::string &database, bool rdOnly = false)
        : GraphIndex(database, rdOnly) {
    ...
    void alignObjects() {
            ...
            for (size_t id = 1; id < exist.size(); id++) {
                if (!exist[id]) {
                    ...
                    try {
                        ...
                        std::stringstream msg;
                        msg << "GraphAndTreeIndex::getSeeds: Cannot search for "
                               "tree.:"
                            << err.what();
                        NGTThrowException(msg);
                    }
                    notexist.insert(
                        std::pair<uint32_t, uint32_t>(tso.nodeID.getID(), id));
                    objectCount++;
                }
            }
        }
        ...
        for (size_t i = 0; i < leafNodeIDs.size(); i++) {
            ...
            for (size_t j = 0; j < objects.size(); j++) {
                order.push_back(
                    std::pair<uint32_t, uint32_t>(objects[j].id, objectCount));
                objectCount++;
            }
            auto nei = notexist.equal_range(leafNodeIDs[i].getID());
            for (auto ii = nei.first; ii != nei.second; ++ii) {
                order.push_back(
                    std::pair<uint32_t, uint32_t>((*ii).second, objectCount));
                objectCount++;
            }
        }
    ...
    void loadIndex(const std::string &ifile, bool readOnly) {
        ...
        if (readOnly) {
            if (property.objectAlignment ==
                NGT::Index::Property::ObjectAlignmentTrue) {
                alignObjects();
            }
    ...
    void remove(const ObjectID id, bool force = false) {
        ...
        try {
#ifdef NGT_SHARED_MEMORY_ALLOCATOR
            obj = GraphIndex::objectSpace->allocateObject(
                *GraphIndex::objectSpace->getRepository().get(id));
#else
            ...
            if (force) {
                ...
                std::stringstream msg;
                msg << err.what()
                    << " Even though the object could not be found, the object "
                       "could be removed from the tree and graph if it existed "
                       "in them.";
                NGTThrowException(msg);
            }
            throw err;
        ...
        if (results.size() == 1) {
            try {
                ...
                std::stringstream msg;
                msg << "remove:: cannot remove from tree. id=" << id << " "
                    << err.what();
                NGTThrowException(msg);
            }
        } else {
            ObjectID replaceID =
                id == results[0].id ? results[1].id : results[0].id;
            try {
    ...
    void insert(ObjectID id) {
        ...
        if (((rs.size() > 0) && (rs[0].distance != 0.0)) || rs.size() == 0) {
            ...
            try {
                ...
                std::cerr << "GraphAndTreeIndex::insert: Fatal error"
                          << std::endl;
                std::cerr << err.what() << std::endl;
                return;
            }
    ...
    void createIndex(
        ...
        std::vector<InsertionResult> &ids, double range, size_t threadNumber);
    ...
    void getSeedsFromTree(NGT::SearchContainer &sc, ObjectDistances &seeds) {
        ...
        try {
            ...
            std::stringstream msg;
            msg << "GraphAndTreeIndex::getSeeds: Cannot search for tree.:"
                << err.what();
            NGTThrowException(msg);
        }
        ...
        try {
            ...
            std::stringstream msg;
            msg << "GraphAndTreeIndex::getSeeds: Cannot get a leaf.:"
                << err.what();
            NGTThrowException(msg);
        }
        ...
        if (sc.useAllNodesInLeaf ||
            NeighborhoodGraph::property.seedType ==
                NeighborhoodGraph::SeedTypeAllLeafNodes) {
            return;
        }
        // if seedSize is zero, the result size of the query is used as
        // seedSize.
        size_t seedSize = NeighborhoodGraph::property.seedSize == 0
                              ? sc.size
                              : NeighborhoodGraph::property.seedSize;
        ...
        if (seeds.size() > seedSize) {
            ...
            for (size_t i = seeds.size(); i > seedSize; i--) {
                double random =
                    ((double)rand() + 1.0) / ((double)RAND_MAX + 2.0);
    ...
    size_t getSharedMemorySize(std::ostream &os,
                               SharedMemoryAllocator::GetMemorySizeType t) {
        return GraphIndex::getSharedMemorySize(os, t) +
               DVPTree::getSharedMemorySize(os, t);
    }
...
class Property : public Index::Property, public NeighborhoodGraph::Property {
   public:
    ...
    static void exportProperty(GraphIndex &graphIndex,
                               const std::string &file) {
        NGT::PropertySet prop;
        graphIndex.getGraphIndexProperty().exportProperty(prop);
        graphIndex.getGraphProperty().exportProperty(prop);
...
size_t NGT::Index::append(const std::vector<T> &object) {
    ...
    auto *o =
        getObjectSpace().getRepository().allocateNormalizedPersistentObject(
            object);
    getObjectSpace().getRepository().push_back(
        dynamic_cast<PersistentObject *>(o));
...
size_t NGT::Index::insert(const std::vector<T> &object) {
    ...
    auto *o =
        getObjectSpace().getRepository().allocateNormalizedPersistentObject(
            object);
    size_t oid = getObjectSpace().getRepository().insert(
        dynamic_cast<PersistentObject *>(o));
    return oid;
}
