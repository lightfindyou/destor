...
int LeafNode::selectPivotByMaxDistance(Container &c, Node::Objects &fs) {
    ...
    for (int i = 1; i < fsize; i++) {
        Distance d = iobj.vptree->objectSpace->getComparator()(*fs[0].object,
                                                               *fs[i].object);
        if (d >= maxd) {
            maxd = d;
            maxid = i;
    ...
    for (int i = 0; i < fsize; i++) {
        Distance d = iobj.vptree->objectSpace->getComparator()(*fs[aid].object,
                                                               *fs[i].object);
        if (i == aid) {
            continue;
        }
    ...
    maxid = 0;
    for (int i = 0; i < fsize; i++) {
        Distance d = iobj.vptree->objectSpace->getComparator()(*fs[bid].object,
                                                               *fs[i].object);
        if (i == bid) {
            continue;
        }
...
int LeafNode::selectPivotByMaxVariance(Container &c, Node::Objects &fs) {
    ...
    for (int i = 0; i < fsize; i++) {
        for (int j = i + 1; j < fsize; j++) {
            Distance d = iobj.vptree->objectSpace->getComparator()(
                *fs[i].object, *fs[j].object);
...
void LeafNode::splitObjects(Container &c, Objects &fs, int pv) {
    ...
    if (cid != 0) {
        ...
        stringstream msg;
        msg << "LeafNode::splitObjects: Too many same distances. Reduce "
               "internal children size for the tree index or not use the tree "
               "index."
            << endl;
        ...
        if (fs[fsize - 1].clusterID == cid) {
            msg << "LeafNode::splitObjects: All of the object distances are "
                   "the same!"
                << endl;
            ;
            ...
            //      cerr << msg.str() << endl;
            //      cerr << "LeafNode::splitObjects: Anyway, continue..." <<
            //      endl;
    ...
    for (int i = 0; i < fsize; i++) {
        if (pivots[fs[i].clusterID] == -1) {
            ...
            Distance d = iobj.vptree->objectSpace->getComparator()(
                *fs[pivots[fs[i].clusterID]].object, *fs[i].object);
            fs[i].leafDistance = d;
        }
    }
    delete[] pivots;
...
LeafNode::removeObject(size_t id, size_t replaceId) {
    ...
    if (idx == fsize) {
        if (pivot == 0) {
            NGTThrowException(
                "LeafNode::removeObject: Internal error!. the pivot is "
                "illegal.");
        }
        stringstream msg;
        msg << "VpTree::Leaf::remove: Warning. Cannot find the specified "
               "object. ID="
            << id << "," << replaceId << " idx=" << idx
            << " If the same objects were inserted into the index, ignore this "
               "message.";
        NGTThrowException(msg.str());
    }
...
bool InternalNode::verify(PersistentRepository<InternalNode> &internalNodes,
                          ...
                          SharedMemoryAllocator &allocator) {
#else
bool InternalNode::verify(Repository<InternalNode> &internalNodes,
    ...
    for (size_t i = 0; i < childrenSize; i++) {
        ...
        if (nid >= size) {
            cerr << "Error! Internal children node id is too big." << nid << ":"
                 << size << endl;
            valid = false;
        }
        try {
            ...
            cerr << "Error! Cannot get the node. "
                 << ((type == ID::Leaf) ? "Leaf" : "Internal") << endl;
            valid = false;
        }
    }
    return valid;
...
#endif
        if (nid > nobjs) {
            cerr << "Error! Object id is too big. " << nid << ":" << nobjs
                 << endl;
            valid = false;
            continue;
        }
