...
NGT::Index::Index(NGT::Property &prop, const string &database) {
    ...
    if (prop.indexType == NGT::Index::Property::GraphAndTree) {
        ...
        NGTThrowException(
            "Index::Index: Not found IndexType in property file.");
    }
    if (idx == 0) {
        stringstream msg;
...
float NGT::Index::getEpsilonFromExpectedAccuracy(double accuracy) {
    return static_cast<NGT::GraphIndex &>(getIndex())
        .getEpsilonFromExpectedAccuracy(accuracy);
}

void NGT::Index::open(const string &database, bool rdOnly) {
    NGT::Property prop;
    prop.load(database);
    Index *idx = 0;
    if (prop.indexType == NGT::Index::Property::GraphAndTree) {
...
void NGT::Index::createGraphAndTree(const string &database, NGT::Property &prop,
    ...
    if (prop.dimension == 0) {
        NGTThrowException(
            "Index::createGraphAndTree. Dimension is not specified.");
    }
...
void NGT::Index::createGraph(const string &database, NGT::Property &prop,
                             const string &dataFile, size_t dataSize,
                             bool redirect) {
    if (prop.dimension == 0) {
        NGTThrowException(
...
void NGT::Index::loadAndCreateIndex(Index &index, const string &database,
                                    const string &dataFile, size_t threadSize,
                                    size_t dataSize) {
    NGT::Timer timer;
    timer.start();
    if (dataFile.size() != 0) {
    ...
    timer.stop();
    cerr << "Data loading time=" << timer.time << " (sec) "
         << timer.time * 1000.0 << " (msec)" << endl;
    ...
    timer.stop();
    index.saveIndex(database);
    cerr << "Index creation time=" << timer.time << " (sec) "
         << timer.time * 1000.0 << " (msec)" << endl;
}
...
void NGT::Index::append(const string &database, const float *data,
                        size_t dataSize, size_t threadSize) {
    NGT::Index index(database);
    NGT::Timer timer;
    timer.start();
...
void NGT::Index::remove(const string &database, vector<ObjectID> &objects,
                        bool force) {
    NGT::Index index(database);
    NGT::Timer timer;
    timer.start();
    for (vector<ObjectID>::iterator i = objects.begin(); i != objects.end();
         i++) {
        try {
            index.remove(*i, force);
        } catch (Exception &err) {
            cerr << "Warning: Cannot remove the node. ID=" << *i << " : "
                 << err.what() << endl;
            continue;
        }
    }
    timer.stop();
    cerr << "Data removing time=" << timer.time << " (sec) "
         << timer.time * 1000.0 << " (msec)" << endl;
    cerr << "# of objects=" << index.getObjectRepositorySize() - 1 << endl;
    index.saveIndex(database);
    return;
...
void NGT::Index::importIndex(const string &database, const string &file) {
    ...
    property.databaseType =
        NGT::Index::Property::DatabaseType::MemoryMappedFile;
    mkdir(database);
#else
    ...
    timer.stop();
    cerr << "Data importing time=" << timer.time << " (sec) "
         << timer.time * 1000.0 << " (msec)" << endl;
...
void NGT::Index::exportIndex(const string &database, const string &file) {
    ...
    timer.stop();
    cerr << "Data exporting time=" << timer.time << " (sec) "
         << timer.time * 1000.0 << " (msec)" << endl;
    cerr << "# of objects=" << idx.getObjectRepositorySize() - 1 << endl;
}
...
typedef NGT::ThreadPool<CreateIndexJob, CreateIndexSharedData *,
                        CreateIndexThread>
    CreateIndexThreadPool;

int CreateIndexThread::run() {
    NGT::ThreadPool<CreateIndexJob, CreateIndexSharedData *,
                    CreateIndexThread>::Thread &poolThread =
        (NGT::ThreadPool<CreateIndexJob, CreateIndexSharedData *,
                         CreateIndexThread>::Thread &)*this;
    ...
    for (;;) {
        ...
        try {
            ...
            cerr << "CreateIndex::search:Error! popFront " << err.what()
                 << endl;
            break;
        }
        ...
        try {
            if (graphIndex.NeighborhoodGraph::property.graphType ==
                NeighborhoodGraph::GraphTypeKNNG) {
                graphIndex.searchForKNNGInsertion(obj, job.id,
                                                  *rs);  // linear search
            } else {
                graphIndex.searchForNNGInsertion(obj, *rs);
            }
        } catch (NGT::Exception &err) {
            cerr << "CreateIndex::search:Fatal error! ID=" << job.id << " "
                 << err.what() << endl;
            abort();
        }
        job.results = rs;
...
class BuildTimeController {
   public:
    BuildTimeController(GraphIndex &graph, NeighborhoodGraph::Property &prop)
        : property(prop) {
        noOfInsertedObjects =
            graph.objectSpace->getRepository().size() - graph.repository.size();
        interval = 10000;
        count = interval;
    ...
    void adjustEdgeSize(size_t c) {
        if (buildTimeLimit > 0.0 && count <= c) {
            ...
            double estimatedTime =
                time + timer.time / interval * (noOfInsertedObjects - count);
            ...
            if (estimatedTime > buildTimeLimit) {
                if (property.insertionRadiusCoefficient - radiusInterval >=
                    1.0) {
...
void NGT::GraphIndex::constructObjectSpace(NGT::Property &prop) {
    ...
    switch (prop.objectType) {
        case NGT::ObjectSpace::ObjectType::Float:
            objectSpace = new ObjectSpaceRepository<float, double>(
                dimension, typeid(float), prop.distanceType);
            break;
        case NGT::ObjectSpace::ObjectType::Uint8:
            objectSpace = new ObjectSpaceRepository<unsigned char, int>(
                dimension, typeid(uint8_t), prop.distanceType);
            break;
        default:
            stringstream msg;
...
void NGT::GraphIndex::loadIndex(const string &ifile, bool readOnly) {
    objectSpace->deserialize(ifile + "/obj");
#ifdef NGT_GRAPH_READ_ONLY_GRAPH
    if (readOnly &&
        property.indexType == NGT::Index::Property::IndexType::Graph) {
...
#ifdef NGT_SHARED_MEMORY_ALLOCATOR
NGT::GraphIndex::GraphIndex(const string &allocator, bool rdonly)
    : readOnly(rdonly) {
    NGT::Property prop;
    prop.load(allocator);
    if (prop.databaseType !=
        NGT::Index::Property::DatabaseType::MemoryMappedFile) {
        NGTThrowException(
            "GraphIndex: Cannot open. Not memory mapped file type.");
    }
    initialize(allocator, prop);
#ifdef NGT_GRAPH_READ_ONLY_GRAPH
    searchUnupdatableGraph = NeighborhoodGraph::Search::getMethod(
        prop.distanceType, prop.objectType,
        objectSpace->getRepository().size());
#endif
}

NGT::GraphAndTreeIndex::GraphAndTreeIndex(const string &allocator,
                                          NGT::Property &prop)
...
NGT::GraphIndex::GraphIndex(const string &database, bool rdOnly)
    : readOnly(rdOnly) {
    NGT::Property prop;
    prop.load(database);
    ...
    if (prop.searchType == "Large") {
        searchUnupdatableGraph = NeighborhoodGraph::Search::getMethod(
            prop.distanceType, prop.objectType, 10000000);
    } else if (prop.searchType == "Small") {
        searchUnupdatableGraph = NeighborhoodGraph::Search::getMethod(
            prop.distanceType, prop.objectType, 0);
    } else {
        searchUnupdatableGraph = NeighborhoodGraph::Search::getMethod(
            prop.distanceType, prop.objectType,
            objectSpace->getRepository().size());
...
static size_t searchMultipleQueryForCreation(GraphIndex &neighborhoodGraph,
                                             NGT::ObjectID &id,
                                             CreateIndexJob &job,
    ...
    for (; id < repo.size(); id++) {
        ...
        if (neighborhoodGraph.NeighborhoodGraph::property.graphType !=
            NeighborhoodGraph::GraphTypeBKNNG) {
            if (id < anngRepo.size() && anngRepo[id] != 0) {
                continue;
            }
        }
        ...
        cnt++;
        if (cnt >= (size_t)neighborhoodGraph.NeighborhoodGraph::property
...
static void insertMultipleSearchResults(
    GraphIndex &neighborhoodGraph,
    CreateIndexThreadPool::OutputJobQueue &output, size_t dataSize) {
...
void GraphIndex::createIndex(size_t threadPoolSize, size_t sizeOfRepository) {
    ...
    if (threadPoolSize <= 1) {
        ...
        BuildTimeController buildTimeController(*this,
                                                NeighborhoodGraph::property);
        ...
        try {
            ...
            for (;;) {
                ...
                size_t cnt = searchMultipleQueryForCreation(
                    *this, id, job, threads, sizeOfRepository);
                if (cnt == 0) {
                    break;
                }
                ...
                if (output.size() != cnt) {
                    cerr << "NNTGIndex::insertGraphIndexByThread: Warning!! "
                            "Thread response size is wrong."
                         << endl;
                    cnt = output.size();
                }
                // insertion
...
#ifdef NGT_SHARED_MEMORY_ALLOCATOR
                    GraphIndex::objectSpace->deleteObject(
                        output.front().object);
#endif
                    output.pop_front();
                }
                ...
                if (timerCount <= count) {
                    timer.stop();
                    cerr << "Processed " << timerCount << " time= " << timer
                         << endl;
...
void GraphIndex::setupPrefetch(NGT::Property &prop) {
    ...
    prop.prefetchOffset =
        GraphIndex::objectSpace->setPrefetchOffset(prop.prefetchOffset);
    prop.prefetchSize =
        GraphIndex::objectSpace->setPrefetchSize(prop.prefetchSize);
}

bool NGT::GraphIndex::showStatisticsOfGraph(NGT::GraphIndex &outGraph,
                                            char mode, size_t edgeSize) {
    ...
    for (size_t id = 1; id < graph.size(); id++) {
        ...
        try {
            ...
            std::cerr << "ngt info: Error. Cannot get the node. ID=" << id
                      << ":" << err.what() << std::endl;
            valid = false;
            continue;
        }
        ...
        for (size_t id = 1; id < graph.size(); id++) {
            ...
            for (size_t i = 0; i < node.size(); i++) {
                ...
                try {
                    ...
                    count++;
#if defined(NGT_SHARED_MEMORY_ALLOCATOR)
                    std::cerr << "Directed edge! " << id << "->"
                              << node.at(i, graph.allocator).id
                              << " no object. "
                              << node.at(i, graph.allocator).id << std::endl;
#else
                    std::cerr << "Directed edge! " << id << "->" << node[i].id
                              << " no object. " << node[i].id << std::endl;
#endif
                    continue;
                }
...
#if defined(NGT_SHARED_MEMORY_ALLOCATOR)
                    std::cerr << "Directed edge! " << id << "->"
                              << node.at(i, graph.allocator).id << " no edge. "
                              << node.at(i, graph.allocator).id << "->" << id
                              << std::endl;
#else
                    std::cerr << "Directed edge! " << id << "->" << node[i].id
                              << " no edge. " << node[i].id << "->" << id
                              << std::endl;
#endif
                    count++;
                }
    ...
    for (size_t id = 1; id < graph.size(); id++) {
        ...
        try {
            ...
            std::cerr << "ngt info: Warning. Cannot get the node. ID=" << id
                      << ":" << err.what() << std::endl;
            continue;
        }
        NGT::GraphNode &node = *n;
    ...
    // calculate variance
    double averageNumberOfOutdegree =
        (double)numberOfOutdegree / (double)numberOfNodes;
    ...
    for (size_t id = 1; id < graph.size(); id++) {
        ...
        sumOfSquareOfOutdegree += ((double)esize - averageNumberOfOutdegree) *
                                  ((double)esize - averageNumberOfOutdegree);
        sumOfSquareOfIndegree +=
    ...
    for (size_t id = 1; id < graph.size(); id++) {
        ...
        if (indegreeCount[id] == 0) {
            ...
            std::cerr << "Error! The node without incoming edges. " << id
                      << std::endl;
            valid = false;
        }
    ...
    std::cerr << "The size of the object repository (not the number of the "
                 "objects):\t"
              << repo.size() - 1 << std::endl;
    ...
    std::cerr << "The mean of the edge lengths:\t" << std::setprecision(10)
              << distance / (double)numberOfOutdegree << std::endl;
    std::cerr << "The mean of the number of the edges per node:\t"
              << (double)numberOfOutdegree / (double)numberOfNodes << std::endl;
    std::cerr << "The number of the nodes without edges:\t"
              << numberOfNodesWithoutEdges << std::endl;
    std::cerr << "The maximum of the outdegrees:\t" << maxNumberOfOutdegree
              << std::endl;
    if (minNumberOfOutdegree == SIZE_MAX) {
        std::cerr << "The minimum of the outdegrees:\t-NA-" << std::endl;
    } else {
        std::cerr << "The minimum of the outdegrees:\t" << minNumberOfOutdegree
                  << std::endl;
    }
    std::cerr << "The number of the nodes where indegree is 0:\t"
              << numberOfNodesWithoutIndegree << std::endl;
    std::cerr << "The maximum of the indegrees:\t" << maxNumberOfIndegree
              << std::endl;
    if (minNumberOfIndegree == INT64_MAX) {
        std::cerr << "The minimum of the indegrees:\t-NA-" << std::endl;
    } else {
        std::cerr << "The minimum of the indegrees:\t" << minNumberOfIndegree
                  << std::endl;
    }
    std::cerr << "#-nodes,#-edges,#-no-indegree,avg-edges,avg-dist,max-out,min-"
                 "out,v-out,max-in,min-in,v-in,med-out,"
                 "med-in,mode-out,mode-in,c95,c5,o-distance(10),o-skip,i-"
                 "distance(10),i-skip:"
              << numberOfNodes << ":" << numberOfOutdegree << ":"
              << numberOfNodesWithoutIndegree << ":" << std::setprecision(10)
              << (double)numberOfOutdegree / (double)numberOfNodes << ":"
              ...
              << medianOutdegree << ":" << medianIndegree << ":"
              << modeOutdegree << ":" << modeIndegree << ":" << c95 << ":" << c5
              << ":" << c99 << ":" << c1 << ":" << distance10 << ":"
              << d10SkipCount << ":" << indegreeDistance10 << ":"
              << ind10SkipCount << std::endl;
    if (mode == 'h') {
        std::cerr << "#\tout\tin" << std::endl;
        for (size_t i = 0;
             i < outdegreeHistogram.size() || i < indegreeHistogram.size();
             i++) {
            size_t out =
                outdegreeHistogram.size() <= i ? 0 : outdegreeHistogram[i];
            size_t in =
                indegreeHistogram.size() <= i ? 0 : indegreeHistogram[i];
...
#ifdef NGT_SHARED_MEMORY_ALLOCATOR
                    Object *f =
                        GraphIndex::objectSpace->allocateObject(*job.object);
...
void GraphAndTreeIndex::createIndex(
    const vector<pair<NGT::Object *, size_t> > &objects,
    vector<InsertionResult> &ids, double range, size_t threadPoolSize) {
    Timer timer;
    size_t timerInterval = 100000;
    size_t timerCount = timerInterval;
    size_t count = 0;
            ...
            for (;;) {
                size_t cnt = 0;
                {
                    for (; idx < objects.size(); idx++) {
                        ...
                        threads.pushInputQueue(job);
                        cnt++;
                        if (cnt >= (size_t)NeighborhoodGraph::property
                                       .batchSizeForCreation) {
                            idx++;
                            break;
                        }
                ...
                {
                    // This processing occupies about 30% of total indexing time
                    // when batch size is 200. Only initial batch objects should
                    // be connected for each other. The number of nodes in the
                    // graph is checked to know whether the batch is initial.
                    size_t size =
                        NeighborhoodGraph::property.edgeSizeForCreation;
                    // add distances from a current object to subsequence
                    // objects to imitate of sequential insertion.
                    ...
                    for (size_t idxi = 0; idxi < cnt; idxi++) {
                        ...
                        for (size_t idxj = 0; idxj < idxi; idxj++) {
                            ...
                            ObjectDistance r;
                            r.distance =
                                GraphIndex::objectSpace->getComparator()(
                                    *output[idxi].object, *output[idxj].object);
                            r.id = output[idxj].id;
                            objs.push_back(r);
                        }
                        ...
                        if ((objs.size() > 0) &&
                            (range < 0.0 || ((double)objs[0].distance <=
                                             range + FLT_EPSILON))) {
                            // The line below was replaced by the line above to
                            // consider EPSILON for float comparison. 170702 if
                            // ((objs.size() > 0) && (range < 0.0 ||
                            // (objs[0].distance <= range))) { An identical or
                            // similar object already exits
                            ids[output[idxi].batchIdx].identical = true;
                            ids[output[idxi].batchIdx].id = objs[0].id;
                            ids[output[idxi].batchIdx].distance =
                                objs[0].distance;
                            output[idxi].id = 0;
                        } else {
                            ...
                            PersistentObject *obj =
                                GraphIndex::objectSpace
                                    ->allocatePersistentObject(
                                        *output[idxi].object);
                            output[idxi].id =
                                GraphIndex::objectSpace->insert(obj);
#else
                            output[idxi].id = GraphIndex::objectSpace->insert(
                                output[idxi].object);
#endif
                    ...
                    if (job.id != 0) {
                        if (property.indexType == NGT::Property::GraphAndTree) {
#ifdef NGT_SHARED_MEMORY_ALLOCATOR
                            Object *f = GraphIndex::objectSpace->allocateObject(
                                *job.object);
                            DVPTree::InsertContainer tiobj(*f, job.id);
#else
                            DVPTree::InsertContainer tiobj(*job.object, job.id);
                            ...
                            try {
                                ...
                                cerr << "NGT::createIndex: Fatal error. ID="
                                     << job.id << ":" << err.what();
#ifdef NGT_SHARED_MEMORY_ALLOCATOR
                                GraphIndex::objectSpace->deleteObject(f);
#endif
                        ...
                        if (((*job.results).size() == 0) && (job.id != 1)) {
                            cerr << "insert warning!! No searched nodes!. If "
...
static bool findPathAmongIdenticalObjects(GraphAndTreeIndex &graph,
                                          size_t srcid, size_t dstid) {
    stack<size_t> nodes;
...
#ifdef NGT_SHARED_MEMORY_ALLOCATOR
        for (auto i = node.begin(graph.repository.allocator);
...
bool GraphAndTreeIndex::verify(vector<uint8_t> &status, bool info, char mode) {
    ...
    bool treeValid = DVPTree::verify(
        GraphIndex::objectSpace->getRepository().size(), status);
    if (!treeValid) {
    ...
    for (size_t id = 1; id < status.size(); id++) {
        ...
        if (status[id] != 0x00 && status[id] != 0x07) {
            if (status[id] == 0x03) {
#ifdef NGT_SHARED_MEMORY_ALLOCATOR
                NGT::Object *po = GraphIndex::objectSpace->allocateObject(
                    *GraphIndex::getObjectRepository().get(id));
...
#endif
                    cerr << "Fatal Error!: Cannot search! " << err.what()
                         << endl;
                    objects.clear();
                }
                size_t n = 0;
                ...
                if (!registeredIdenticalObject) {
                    if (info) {
                        cerr << "info: not found the registered same objects. "
                                "id="
                             << id << " size=" << objects.size() << endl;
                    }
                    sc.id = 0;
                    sc.radius = FLT_MAX;
                    ...
                    for (n = 0; n < objects.size(); n++) {
                        ...
                        if (objects[n].id != id &&
                            status[objects[n].id] == 0x07) {
                            registeredIdenticalObject = true;
                            if (info) {
                                cerr << "info: found by using mode accurate "
                                        "search. "
                                     << objects[n].id << endl;
                            }
                            break;
                ...
                if (!registeredIdenticalObject && mode != 's') {
                    if (info) {
                        cerr << "info: not found by using more accurate search."
                             << endl;
                    }
                    sc.id = 0;
                    sc.radius = 0.0;
                    ...
                    for (; n < objects.size(); n++) {
                        ...
                        if (objects[n].id != id &&
                            ...
                            registeredIdenticalObject = true;
                            if (info) {
                                cerr << "info: found by using linear search. "
                                     << objects[n].id << endl;
                            }
                            break;
                        }
                ...
                if (registeredIdenticalObject) {
                    if (info) {
                        cerr << "Info ID=" << id << ":"
                             << static_cast<int>(status[id]) << endl;
                        cerr << "  found the valid same objects. "
                             << objects[n].id << endl;
                    }
...
#ifdef NGT_SHARED_MEMORY_ALLOCATOR
                    for (auto i =
                             fromNode.begin(GraphIndex::repository.allocator);
                         i != fromNode.end(GraphIndex::repository.allocator);
                         ++i) {
#else
...
#ifdef NGT_SHARED_MEMORY_ALLOCATOR
                    for (auto i =
                             toNode.begin(GraphIndex::repository.allocator);
                         i != toNode.end(GraphIndex::repository.allocator);
                         ++i) {
#else
                    ...
                    if (!fromFound || !toFound) {
                        if (info) {
                            if (!fromFound && !toFound) {
                                cerr << "Warning no undirected edge between "
                                     << id << "(" << fromNode.size() << ") and "
                                     << objects[n].id << "(" << toNode.size()
                                     << ")." << endl;
                            } else if (!fromFound) {
                                cerr << "Warning no directed edge from " << id
                                     << "(" << fromNode.size() << ") to "
                                     << objects[n].id << "(" << toNode.size()
                                     << ")." << endl;
                            } else if (!toFound) {
                                cerr << "Warning no reverse directed edge from "
                                     << id << "(" << fromNode.size() << ") to "
                                     << objects[n].id << "(" << toNode.size()
                                     << ")." << endl;
                            }
                        }
                        if (!findPathAmongIdenticalObjects(*this, id,
                                                           objects[n].id)) {
                            cerr << "Warning no path from " << id << " to "
                                 << objects[n].id << endl;
                        }
                        if (!findPathAmongIdenticalObjects(*this, objects[n].id,
                                                           id)) {
                            cerr << "Warning no reverse path from " << id
                                 << " to " << objects[n].id << endl;
                        }
                    }
                } else {
                    if (mode == 's') {
                        cerr << "Warning: not found the valid same object, but "
                                "not try to use linear search."
                             << endl;
                        cerr << "Error! ID=" << id << ":"
                             << static_cast<int>(status[id]) << endl;
                    } else {
                        cerr << "Warning: not found the valid same object even "
