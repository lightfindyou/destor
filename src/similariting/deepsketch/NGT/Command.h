...
class Command {
   public:
    class SearchParameter {
       public:
        ...
        void parse(Args &args) {
            ...
            size = args.getl("n", 20);
            // edgeSize
            // -1(default) : using the size which was specified at the index
            // creation.
            ...
            trial = args.getl("t", 1);
            {
                ...
                if (tokens.size() >= 1) {
                    beginOfEpsilon = endOfEpsilon =
                        NGT::Common::strtod(tokens[0]);
                }
                if (tokens.size() >= 2) {
    ...
    static void search(NGT::Index &index, SearchParameter &searchParameter,
                       std::ostream &stream) {
        std::ifstream is(searchParameter.query);
        if (!is) {
            std::cerr << "Cannot open the specified file. "
                      << searchParameter.query << std::endl;
            return;
        }
        search(index, searchParameter, is, stream);
    }
    static void search(NGT::Index &index, SearchParameter &searchParameter,
                       std::istream &is, std::ostream &stream);
